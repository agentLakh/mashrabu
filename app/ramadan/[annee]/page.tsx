...
async function getData(annee: number) {
  const { data: edition } = await supabase.from('editions').select('*').eq('annee', annee).eq('categorie', 'ramadan').single();
  if (!edition) return null;
  const { data: jours } = await supabase.from('jours').select('*').eq('edition_id', edition.id).order('numero', { ascending: true });
  if (!jours) return { edition, jours: [] };
  const joursWithCount = await Promise.all(jours.map(async (jour) => {
    const { count } = await supabase.from('sons').select('*', { count: 'exact', head: true }).eq('jour_id', jour.id);
    return { ...jour, sonsCount: count ?? 0 };
  }));
  return { edition, jours: joursWithCount };
}

export default async function RamadanEditionPage({ params }: { params: { annee: string } }) {
  ...
  return (
    <main className="min-h-screen relative text-white" style={{ background: 'linear-gradient(135deg, #022c22 0%, #064e3b 50%, #065f46 100%)' }}>
      ...
      <div className="relative z-10 container mx-auto px-4 py-12 max-w-7xl">
        <Link href="/ramadan" className="inline-flex items-center text-emerald-300 hover:text-amber-400 mb-8 transition-colors mt-16 md:mt-0">
          ...
          <span className="font-medium">Retour aux éditions</span>
        </Link>
        <header className="text-center mb-16 space-y-4">
          <h1 className="arabic-text text-5xl md:text-7xl font-bold text-amber-400 mb-4 leading-tight" style={{ textShadow: '0 4px 20px rgba(251,191,36,0.3)' }}>
            مشرب صافي<br /><span className="text-emerald-400">حزب الترقية {edition.annee}</span>
          </h1>
          <p className="text-xl md:text-2xl text-emerald-300 font-light tracking-wide">{edition.titre}</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
          {jours.map((jour: any) => {
            ...
            return (
              <Link key={jour.id} href={`/ramadan/${annee}/jour/${jour.numero}`} className="day-card group block rounded-2xl p-6 cursor-pointer relative overflow-hidden">
