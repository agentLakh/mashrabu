    ...
    <main className="min-h-screen relative text-white" style={{ background: 'linear-gradient(135deg, #022c22 0%, #064e3b 50%, #065f46 100%)' }}>
      ...
      <div className="relative z-10 container mx-auto px-4 py-12 max-w-4xl">
        ...
        <header className="text-center mb-16 space-y-4">
          ...
          <p className="text-xl md:text-2xl text-emerald-300 font-light tracking-wide">Magal de Touba — Hizbut-Tarqiyyah</p>
          <p className="text-emerald-400/70 text-sm">Sélectionne une édition pour accéder aux kourels</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {editions.map((edition: any) => (
            <Link key={edition.id} href={`/magal/${edition.annee}`} className="day-card group block rounded-2xl p-8 cursor-pointer relative overflow-hidden text-center">
          ...
          {editions.length === 0 && (
            <div className="col-span-3 text-center text-emerald-400/50 py-20">Aucune édition Magal disponible.</div>
          )}
        </div>

        <footer className="mt-20 text-center text-emerald-400/50 text-sm">
          © Magal de Touba — Hizbut-Tarqiyyah — Tous droits réservés
        </footer>
      </div>
    </main>
  );
