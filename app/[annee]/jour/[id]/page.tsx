...
async function getJourData(annee: number, numero: number) {
  const { data: edition } = await supabase
    .from('editions')
    .select('id, annee, titre')
    .eq('annee', annee)
    .single();
...
export default async function JourPage({ params }: { params: { annee: string; id: string } }) {
  ...
  const data = await getJourData(annee, numero);
  if (!data) notFound();
  ...
  return (
    <main className="min-h-screen relative" style={{ background: 'linear-gradient(135deg, #022c22 0%, #064e3b 50%, #065f46 100%)', color: 'white' }}>
      ...
      <div className="relative z-10 container mx-auto px-4 py-8 max-w-5xl">
        {/* Back */}
        <Link href={`/${annee}`} className="inline-flex items-center text-emerald-300 hover:text-amber-400 mb-8 transition-colors group">
          ...
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            <div className="flex-1">
              ...
              <p className="text-emerald-300 mt-4 text-sm flex items-center gap-1">
                ...
                Programme du jour — Édition {edition.annee}
              </p>
            </div>
